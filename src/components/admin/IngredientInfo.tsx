...
// Extend the data ingredient with UI fields
...
export const IngredientInfo = () => {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: 'name' | 'category' | 'protein' | 'cost'; direction: 'asc' | 'desc' } | null>(null);
  const [expandedId, setExpandedId] = useState<string | null>(null);

  // Fetch and combine data only once
  ...
  const requestSort = (key: 'name' | 'category' | 'protein' | 'cost') => {
  ...
  return (
    <div className="space-y-6">
              ...
              <tr>
                {['name','category','protein','cost'].map(key => (
                  <th
                    ...
                    onClick={() => requestSort(key as any)}
                  >
                    <div className="flex items-center">
                      {key === 'name' ? 'Ingredient' : key.charAt(0).toUpperCase() + key.slice(1)}
                      {sortConfig?.key === key && (
            ...
            <tbody className="divide-y divide-gray-700">
              {filtered.map(item => (
                <>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-200">{item.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{item.category?.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-400">
                      {item.compositions.find(c => c.nutrient?.name === 'Crude Protein')?.value}%
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-200 font-medium">${item.cost.toFixed(2)}</td>
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              {['Crude Protein','Fat (Ether Extract)','Crude Fiber'].map(n => {
                                const comp = item.compositions.find(c => c.nutrient?.name === n);
                                return (
                          ...
                          <div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="text-gray-200">${(item.cost / (item.compositions.find(c => c.nutrient?.name === 'Crude Protein')?.value || 1)).toFixed(3)}/%</span>
                              </div>
                            </div>
                          </div>
      ...
      {/* Add Ingredient Modal (unchanged) */}
      {showForm && (
